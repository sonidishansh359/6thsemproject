...
router.put('/:id/status', ownerAuth, async (req, res) => {
  try {
    ...
    const allowedOwnerStatuses = ['placed', 'accepted', 'preparing', 'out_for_delivery', 'delivered'];
    ...
    if (req.body.status === 'out_for_delivery') {
      try {
        console.log(`💰 Owner marked as out_for_delivery - processing payout from Admin...`);

        const Admin = require('../models/Admin');
        const Transaction = require('../models/Transaction');
        const User = require('../models/User');

        const earningAmount = order.ownerEarning || order.totalAmount;
        ...
        if (!adminUser) throw new Error('Admin user not found for payout');

        let admin = await Admin.findOne({ user: adminUser._id });
        if (!admin) {
          console.error('❌ Admin wallet not found!');
          return res.status(500).json({ message: 'System Error: Admin wallet not initialized.' });
        }

        // Ensure admin has enough balance
        if ((admin.availableBalance || 0) < earningAmount) {
          console.error(`❌ Insufficient Admin Funds. Required: ${earningAmount}, Available: ${admin.availableBalance}`);
          return res.status(400).json({
            message: 'System Error: Insufficient funds in Admin Wallet to process payout.'
          });
        }

        // 2. Find Owner (already verified by ownerAuth but for safety)
        const restaurantDoc = await Restaurant.findById(order.restaurant);
        const owner = await Owner.findOne({ user: restaurantDoc.owner });
        // Note: req.owner is likely the OWNER MODEL instance from middleware, but let's be sure.
        // wait, middleware usually attaches User. 
        // In ownerAuth: const owner = await Owner.findOne({ user: req.user.id }); req.owner = owner;
        // So req.owner is correct.

        // Check for duplicate payout
        const existingTx = await Transaction.findOne({
          referenceId: order._id.toString(),
          type: 'earning', // Owner earning
          description: { $regex: 'Earnings from Order' } // Looser check just in case
        });

        if (!existingTx) {
          // 3. Deduct from Admin
          const oldBalance = admin.availableBalance;
          admin.availableBalance -= earningAmount;
          await admin.save();
          console.log(`💰 Admin Payout: -${earningAmount}. Balance: ${oldBalance} -> ${admin.availableBalance}`);

          // 4. Credit Owner
          const ownerInstance = req.owner; // Use middleware instance
          ownerInstance.availableBalance = (ownerInstance.availableBalance || 0) + earningAmount;
          await ownerInstance.save();

          console.log(`✅ Owner Credited: +${earningAmount}. New Balance: ${ownerInstance.availableBalance}`);

          // 5. Create Transactions
          // Owner Credit Log
          await Transaction.create({
            owner: ownerInstance._id, // This expects Owner ID, not User ID. req.owner is Owner model.
          ...
          console.log('⚠️ Payout already processed for this order.');
        }

      } catch (err) {
        console.error('❌ Error processing owner payout:', err);
        return res.status(500).json({ message: 'Payout processing failed', error: err.message });
      }
    }
